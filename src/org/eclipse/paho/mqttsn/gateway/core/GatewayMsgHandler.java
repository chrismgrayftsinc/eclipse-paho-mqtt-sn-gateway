...
public class GatewayMsgHandler extends MsgHandler {
      ...
      default:
        GatewayLogger.log(GatewayLogger.WARN,
            "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                + "]/[" + clientId + "] - Mqtts message of unknown type \"" + receivedMsg
                .getMsgType() + "\" received.");
        break;
    }
  }
  ...
  private void handleMqttsAdvertise(MqttsAdvertise receivedMsg) {
    ...
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtts ADVERTISE message received.");
  }
  ...
  private void handleMqttsGWInfo(MqttsGWInfo receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtts GWINFO message received.");
  ...
  private void handleMqttsPublish(MqttsPublish receivedMsg) {
    if (receivedMsg.getTopicIdType() == MqttsMessage.NORMAL_TOPIC_ID) {
      GatewayLogger.log(GatewayLogger.INFO,
          "GatewayMsgHandler [" + Utils.hexString(this.gatewayAddress.getAddress()) + "]/["
              + clientId + "] - Mqtts PUBLISH message with \"QoS\" = \"" + receivedMsg.getQos()
              + "\" and \"TopicId\" = \"" + receivedMsg.getTopicId() + "\" received.");
      ...
      GatewayLogger.log(GatewayLogger.INFO,
          "GatewayMsgHandler [" + Utils.hexString(this.gatewayAddress.getAddress()) + "]/["
              + clientId + "] - Mqtts PUBLISH message with \"QoS\" = \"" + receivedMsg.getQos()
              + "\" and \"TopicId\" = \"" + receivedMsg.getShortTopicName()
    ...
    switch (receivedMsg.getTopicIdType()) {
      ...
      case MqttsMessage.NORMAL_TOPIC_ID:
        GatewayLogger.log(GatewayLogger.WARN,
            "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                + "]/[" + clientId + "] - Topic Id type " + receivedMsg.getTopicIdType()
                + " is invalid. Publish with \"QoS\" = \"-1\" supports only predefined topis Ids (topic Id type = \"1\") or short topic names (topic Id type = \"2\").");
        return;
      ...
      case MqttsMessage.PREDIFINED_TOPIC_ID:
        if (receivedMsg.getTopicId() > GWParameters.getPredfTopicIdSize()) {
          GatewayLogger.log(GatewayLogger.WARN,
              "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                  + "]/[" + clientId + "] - Predefined topicId (\"" + receivedMsg.getTopicId()
        ...
        if (topicName == null) {
          GatewayLogger.log(GatewayLogger.WARN,
              "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                  + "]/[" + clientId + "] - Predefined topicId (\"" + receivedMsg.getTopicId()
                  + "\") of the received Mqtts PUBLISH message does not exist. The message cannot be processed.");
          return;
        }
      ...
      default:
        GatewayLogger.log(GatewayLogger.WARN,
            "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                + "]/[" + clientId + "] - Unknown topicIdType (\"" + receivedMsg.getTopicIdType()
    ...
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(this.gatewayAddress.getAddress()) + "]/[" + clientId
            + "] - Sending Mqtt PUBLISH message with \"QoS\" = \"" + publish.getQos()
    ...
    try {
      ...
      e.printStackTrace();
      GatewayLogger.log(GatewayLogger.ERROR,
          "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
              + "]/[" + clientId + "] - Failed sending Mqtt PUBLISH message to the broker.");
      connectionLost();
    }
  }
  ...
  public void handleMqttMessage(MqttMessage receivedMsg) {
    ...
    switch (receivedMsg.getMsgType()) {
      ...
      default:
        GatewayLogger.log(GatewayLogger.WARN,
            "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                + "]/[" + clientId + "] - Mqtt message of unknown type \"" + receivedMsg
                .getMsgType() + "\" received.");
        break;
    }
  }
  ...
  private void handleMqttConnack(MqttConnack receivedMsg) {
    ...
    if (receivedMsg.getReturnCode() != MqttMessage.RETURN_CODE_CONNECTION_ACCEPTED) {
      GatewayLogger.log(GatewayLogger.ERROR, "GatewayMsgHandler [" +
          Utils.hexString(GWParameters.getGatewayAddress().getAddress()) +
          "]/[" + clientId
          + "] - Return Code of Mqtt CONNACK message it is not \"Connection Accepted\".");
      GatewayLogger.log(GatewayLogger.ERROR, "GatewayMsgHandler [" +
          Utils.hexString(GWParameters.getGatewayAddress().getAddress()) +
          "]/[" + clientId
    ...
    while (st.hasMoreTokens()) {
      ...
      try {
        ...
        GatewayLogger.info(
            "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                + "]/[" + clientId + "] - " + inter.getClass().getName() + " initialized.");
        clientInterfacesVector.add(inter);
        init = true;
      } catch (IllegalAccessException e) {
        GatewayLogger.warn(
            "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                + "]/[" + clientId + "] - Failed to instantiate " + clInte + ".");
        e.printStackTrace();
        ...
        GatewayLogger.warn(
            "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                + "]/[" + clientId + "] - Failed to initialize " + inter.getClass().getName());
        e.printStackTrace();
      }
    }
    ...
    GatewayLogger.info(
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Broadcasting initial Mqtts ADVERTISE message...");
...
//		sendMqttsAdvertise();
    ...
    timer.register(gatewayAddress, ControlMessage.SEND_KEEP_ALIVE_MSG,
        GWParameters.getKeepAlivePeriod());
  ...
  private void handleMqttPingReq(MqttPingReq receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtt PINGREQ message received.");
  ...
  private void handleMqttUnsuback(MqttUnsuback receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtt UNSUBACK message received.");
  ...
  private void handleMqttSuback(MqttSuback receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtt SUBACK message received.");
  ...
  private void handleMqttPubComp(MqttPubComp receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtt PUBCOMP message received.");
  ...
  private void handleMqttPubRel(MqttPubRel receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtt PUBREL message received.");
  ...
  private void handleMqttPubRec(MqttPubRec receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtt PUBREC message received.");
  ...
  private void handleMqttPuback(MqttPuback receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtt PUBACK message received.");
  ...
  private void handleMqttPublish(MqttPublish receivedMsg) {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtt PUBLISH message received.");
  ...
  public void handleControlMessage(ControlMessage receivedMsg) {
    ...
    switch (receivedMsg.getMsgType()) {
      ...
      default:
        GatewayLogger.log(GatewayLogger.WARN,
            "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
                + "]/[" + clientId + "] - Control message of unknown type \"" + receivedMsg
                .getMsgType() + "\" received.");
        break;
    }
  }
  ...
  private void shutDown() {
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Control SHUT_DOWN message received.");
    ...
    GatewayLogger.log(GatewayLogger.INFO,
        "GatewayMsgHandler [" + Utils.hexString(this.gatewayAddress.getAddress()) + "]/[" + clientId
            + "] - Sending Mqtt DISCONNECT message to the broker.");
    try {
  ...
  private void sendMqttPingReq() {
    ...
    try {
      ...
      GatewayLogger.error(
          "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
              + "]/[" + clientId + "] - Failed sending Mqtts PINGREQ message to the broker.");
      connectionLost();
    }
  }
  ...
  private void sendMqttsAdvertise() {
    ...
    GatewayLogger.info(
        "GatewayMsgHandler [" + Utils.hexString(GWParameters.getGatewayAddress().getAddress())
            + "]/[" + clientId + "] - Mqtts ADVERTISE message was broadcasted to the network.");
  }
